...
const graficoTorta = function () {
  ...
  ss.style.strokeDashoffset = 381 + (381 * s) / countDown;
...
const nextQuestion = function () {
  ...
  y += 1;
  if (y == 11) {
    buttonAvanti.innerHTML = '<button onclick="reveal(event)">Risultati</button>';
  }
  const risposte = document.querySelectorAll(".button-answer");
...
const btnInteraction = function (event) {
  ...
  wrong.innerText = percentualeRisposteSbagliate + "%";
  let correct = document.getElementById("correct");
  correct.innerText = percentualeRisposteGiuste + "%";
  let wrongRespond = document.getElementById("risposteSbagliate");
  wrongRespond.innerText = risposteSbagliate.length + "/10 risposte sbagliate";
  let correctRespond = document.getElementById("risposteGiuste");
  correctRespond.innerText = risposteGiuste.length + "/10 risposte giuste";
...
const reveal = function () {
  ...
  let bench = document.getElementById("benchamark");
  bench.classList.add("displayNone");
};
